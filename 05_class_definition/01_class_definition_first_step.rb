...
class MetaClass
  class << self
    def meta_attr_accessor(method_name)
      new_method_name = "meta_#{method_name}"
      define_method "#{new_method_name}=" do |value|
        (@variables ||= {})[method_name] = value
      end

      define_method new_method_name do
        value = @variables[method_name]
        "meta #{value}"
      end
    end
  end
